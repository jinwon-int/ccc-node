...
Environment:
  ...
  SEARXNG_URL        comma-separated SearXNG base URLs for --provider searxng
                     (required for searxng in this fleet-canon copy: no
                     node-specific default is embedded; the current endpoint
                     is documented in the Family Wiki
                     pages/services/searxng.md)
...
# Fleet-canon copy (#1446): the deployment canon must be public-safe and
# fleet-universal, so no node-specific SearXNG endpoint is embedded here.
# Operators set SEARXNG_URL per node; the current endpoint value and its
# ownership/failover runbook live in the Family Wiki
# (pages/services/searxng.md). The Piri-lane copy of this helper keeps a
# lane-local default instead.
DEFAULT_SEARXNG_URL = ""
...
def _search_searxng(query: str, limit: int) -> int:
    ...
    if not bases:
        print(
            "web-search: SEARXNG_URL is not configured for --provider searxng "
            "(this fleet-canon copy embeds no default; see the Family Wiki "
            "pages/services/searxng.md)",
            file=sys.stderr,
        )
        return 64
