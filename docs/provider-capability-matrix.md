...
| `memory_writeback_distill` — Memory: write-back distill: Durable facts are extracted from provider threads for the memory sinks. | `supported` — PreCompact/SessionEnd distill transcripts into resume/local facts and sink candidates via claude/hooks/distill.sh. | `degraded` — Session-reset and explicit triggers, extraction, and the local sink are scheduled; checkpoint triggers and Honcho/Wiki routing remain pending. (depends on #465) |
