...
|---|---|---|
| `runtime_adapter` — Provider-neutral runtime adapter: Sessions and turns run behind the AgentRuntime seam and pass the runtime conformance suite. | `supported` — ClaudeRuntime adapts the Claude Agent SDK to AgentRuntime, passes the runtime conformance suite, and is the default live path since the #584 slice C-1 cutover (CCC_CLAUDE_RUNTIME_ADAPTER=0 is the emergency kill-switch back to the legacy direct SDK path). | `supported` — CodexRuntime adapts the app-server protocol to AgentRuntime and passes the runtime conformance suite. |
