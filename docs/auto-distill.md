...
- when recorded, that `model_resolution` names a concrete provider model id
  for the launcher alias rather than the alias itself (see below).
...
### Resolved model id (`evaluation.model_resolution`, optional)

`evaluation.model` records the alias the evaluation launcher passed
(`--model haiku`). The provider resolves that alias server-side, so two
evaluations weeks apart can run on different models while their logs look
identical; the #1514 read-only investigation could not tell whether TM-2408
(08-26) and TM-3298 (09-05) shared one model id because nothing recorded it
(#1521). A receipt may therefore add an optional object next to `model`:

```json
"model_resolution": {
  "alias": "haiku",
  "resolved_id": "<provider model id>",
  "resolved_by": "claude-json-modelUsage:eval.log",
  "resolved_at": "2026-09-05T13:01:16Z"
}
```

- `alias` must equal `evaluation.model`.
- `resolved_id` is the concrete provider id (1–80 chars of
  `[A-Za-z0-9._:/@-]`). A bare alias (`haiku`, `sonnet`, `opus`, `default`,
  any case) is rejected, and for the `haiku`/`sonnet`/`opus` aliases the id
  must contain that family name.
- `resolved_by` (1–120 chars) says how the id was obtained:
  `claude-json-modelUsage:<artifact basenames>` when read from the evaluation's
  own Claude `--output-format json` envelopes, or `operator` when recorded by
  hand from the provider's response.
- `resolved_at` is a timezone-aware ISO-8601 timestamp no later than
  `issued_at`.

The object is optional for backward compatibility: the current receipt
(TM-3298) predates it and keeps verifying unchanged, with the summary ending in
`model=unrecorded`. When present and valid the summary ends in
`model=<resolved_id>`. Pass `--require-model` to fail (`exit 3`) on a receipt
that omits it; a future re-issued receipt should carry the object. Re-issuing a
receipt means a new exact-source evaluation on the evaluation node and remains
a separately approved action — this verifier change does not re-issue anything.

Resolution never calls a model. `claude --version` prints only the CLI
version and the CLI has no offline alias table, so the only model-call-free
source is the `modelUsage` block of the envelopes the evaluation already wrote
(the same block `auto-distill.py` reads for cost accounting):

```bash
python3 scripts/auto-distill/resolve_model_id.py --envelope /path/to/eval.log
```

prints a receipt-ready `model_resolution` object (exit `1` when the artifacts
carry no id or disagree; exit `2` with the manual-entry instructions when no
envelope is given). Keeping the launcher on a fixed model id instead of an
alias (`run-exact-eval.sh` `AD_MODEL_CMD`) is tracked separately in #1521.
