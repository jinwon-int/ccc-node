...
Provider lifecycle preservation completes that contract without installing Claude hooks in another runtime. Piri's matching `compaction_start` event copies the current state to `$CCC_STATE_DIR/checkpoints/working-state-YYYYMMDD_HHMMSS.md` and retains the newest 30 files. A conversation-local Piri or Codex session close copies it to `$CCC_STATE_DIR/session-archive/working-state-<digest>.md`; closing a Codex conversation does not close its shared app-server. Both paths are best-effort, atomically write owner-private files, cap the source at 64 KiB by default (`CCC_WORKING_STATE_ARCHIVE_MAX_BYTES`, hard maximum 1 MiB), and can be disabled with `CCC_WORKING_STATE_ARCHIVE=0`. Audience-scoped sessions write only to their scoped state tree; only a scoped private audience may read the #1155 legacy fallback, never a shared audience. Codex has no documented pre-compaction notification, so it archives only at the lifecycle boundary it can prove.
