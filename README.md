...
- **Local memory diagnostics/eval** — `scripts/ccc-memory-check.sh`,
  ...
  `scripts/ccc-memory-consolidate.sh`,
...
| `CCC_MEMORY_NO_REFRESH` | unset (refresh fires) | Set `1`/`true`/`on` to suppress the detached background `refresh-memory.sh` that `load-memory.sh` spawns — a strictly read-only inject. Used to make hook tests hermetic (the refresh rebuilds the index / consolidates facts out-of-band, otherwise racing fixtures mid-test) |
...
| `CCC_MEMORY_INJECT_RENDER` | `1` (on) | Render the local hot-memory block as compact `- (source) snippet` lines instead of the raw search JSON (whose per-result score/`signals`/paths are debug noise that wastes the injection budget). The search tool and `ccc-memory-explain` still emit full JSON. Set `0`/`false`/`off` to inject raw JSON |
| `CCC_MEMORY_DYNAMIC_BUDGET` | `1` (on) | Relevance-aware budget: when small/empty MEMORY/wiki/honcho blocks leave injection budget unused (e.g. no wiki/honcho cache, or `max-perf` which drops Honcho), give the slack to the task-conditioned local hot block — growing both its byte budget (above `CCC_LOCAL_MEMORY_MAX_BYTES`) and how many results it fetches to fill it (up to 25). Purely additive (never below the defaults); `CCC_MEMORY_MAX_BYTES` still bounds the whole injection. An explicit `CCC_MEMORY_SEARCH_LIMIT` wins. Set `0`/`false`/`off` to use static caps |
| `CCC_MEMORY_USAGE_FEEDBACK` | `1` (on) | Retrieval-frequency feedback loop: docs repeatedly retrieved for real injections earn a small, recency-decayed `usage_boost` so durably-useful memory surfaces faster. Keyed by content hash in `memory-usage.json` (local, chmod 600, bounded). The boost is read everywhere (no-op until stats exist) but only **recorded** when the caller sets `CCC_MEMORY_RECORD_USAGE=1` (load-memory's injection search), so `ccc-memory-explain`/tests stay read-only. The boost is capped below one token of coverage, so it only breaks ties — a popular doc never outranks a stronger lexical match. Set `0`/`false`/`off` to disable read + write |
| `CCC_MEMORY_USAGE_WEIGHT` / `CCC_MEMORY_USAGE_TTL_DAYS` | `1.5` / `30` | Usage-boost weight (`weight × ln(1+retrievals)`, capped at 3.0) and the recency-decay window over which an unused doc's accumulated boost fades to zero |
| `CCC_MEMORY_CONSOLIDATE` / `CCC_MEMORY_CONSOLIDATE_SIM` | `1` (on) / `0.82` | Background consolidation (`ccc-memory-consolidate.sh`, run by `refresh-memory.sh`) collapses near-duplicate distilled facts: clusters with char-4-gram Jaccard ≥ the threshold (same `kind`) keep the most recent and mark older **machine-generated** copies `review:superseded` (kept as an audit trail, skipped by the index). Human-reviewed facts are never auto-superseded. Set `0`/`false`/`off` to disable |
