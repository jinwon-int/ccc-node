...
- **Managed-services relaxation** (operator-owned allowlist; opt-in, fail-closed).
  ...
  **every** target of the `systemctl`/`service`/`pm2`/`docker`/`podman` lifecycle command
  ...
  this allowlist. Wrappers, substitutions, non-detached `up`, and remote-daemon selection
  do not enter that carve-out. This keeps `sshd`/`ufw`/`nginx` and other system services
  protected while unblocking the node's own apps. Write-gated for agents
