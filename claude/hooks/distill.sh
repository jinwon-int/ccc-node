...
if ! scope_allows_project "$PROJECT_ENC" "$SOURCE_CWD"; then
  log "skip reason=cwd-out-of-scope trigger=$TRIGGER pid=$$ cwd=$SOURCE_CWD project=$PROJECT_ENC"
  exit 0
fi
...
if [ "$TURNS" -lt "$MIN_TURNS" ]; then
  log "skip reason=too-few-turns trigger=$TRIGGER pid=$$ turns=$TURNS min_turns=$MIN_TURNS"
  exit 0
fi
