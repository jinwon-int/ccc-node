...
def latest(path, key):
    if not Path(path).exists():
        if Path(path).is_symlink(): raise ValueError('symlink_receipt')
        return None
    with os.fdopen(private_open(path, os.O_RDONLY), 'r') as f:
        if os.fstat(f.fileno()).st_size > MAX_BYTES: raise ValueError('receipt_capacity')
        found = None
        for line in f:
            try: row = json.loads(line)
            except ValueError: raise ValueError('invalid_receipt') from None
            if row.get('key') == key: found = row
        return found
...
def main(args):
    ...
    if action == 'due':
        if previous and previous.get('fingerprint') == current:
            if previous.get('status') == 'stored': return 3
            if previous.get('attempts', 0) >= 3: return 3
            if now - previous.get('at', 0) < 600: return 3
        elif not previous and rest and Path(rest[0]).is_file():
            with os.fdopen(private_open(rest[0], os.O_RDONLY)) as f:
                if os.fstat(f.fileno()).st_size > MAX_BYTES: raise ValueError('legacy_capacity')
                legacy = source in f.read().splitlines()
            days = max(0, min(3650, int(os.environ.get('NUNCHI_FEED_REPLAY_DAYS', '7'))))
            if legacy and modified < now - days * 86400: return 3
        # Token binds the eventual receipt to the pre-extraction snapshot.
        print(current)
        return 0
    if action not in ('stored', 'failed') or not rest: raise ValueError('arguments')
    expected = rest[0]
    if expected != current: return 4  # source changed while extracting; retry it
    attempts = (previous.get('attempts', 0) if previous and previous.get('fingerprint') == current else 0) + 1
