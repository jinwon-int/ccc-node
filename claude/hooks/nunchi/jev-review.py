...
DENY = re.compile(
    r"가족|아내|남편|자녀|아이|딸|아들|주민|생년|전화|계좌|카드|은행|대출|보증|소송|판결|법률|감사원|HUG|병원|환자|진단|의료|비밀|비밀번호|토큰\s*[:=]|api.?key|password|secret|bearer|cookie|credential|family|medical|patient|lawsuit|salary",
    re.I,
)
TECH = re.compile(
    r"\b(?:ccc|nunchi|jev|codex|claude|piri|danso|git|github|PR|CI|API|SQLite|JSON|systemd|cron|Docker|pytest|SSH)\b|배포|브리지|회귀.?테스트|수집기|스키마",
    re.I,
)
NAMES = re.compile(
    r"서서|노숙|진군|등애|순욱|방통|공명|곽가|육손|소교|대교|공융|서진온|\b(?:seoseo|nosuk|jingun|dungae|soonwook|bangtong|gongmyoung|gwakga|yukson|sogyo|daegyo|gongyung|seo-jin-on|jinwon-int|jinon86)\b",
    re.I,
)
...
def sanitize(text):
    if (
        ...
        or not 20 <= len(text) <= 1200
        or DENY.search(text)
        or not TECH.search(text)
    ):
        return None
    text = re.sub(
        r"https?://\S+|[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}|(?:\d{1,3}\.){3}\d{1,3}|(?:/[^\s,;]+)+|\b[A-Fa-f0-9]{16,}\b",
        "[reference]",
        text,
    )
...
def validate(raw):
    ...
    p = a["probabilities"]
    c = a["confidence"]
    if a.get("type") != "choice" or a.get("choice") not in KINDS or set(p) != KINDS:
...
def request(sentence, key, question):
    data = json.dumps(
        {"model": MODEL, "state": {"sentence": sentence}, "questions": {"kind": question}},
        ensure_ascii=False,
    ).encode()
...
def read_key(keypath):
    with os.fdopen(r.private_open(keypath, os.O_RDONLY)) as f:
        if os.fstat(f.fileno()).st_mode & 0o077:
            raise ValueError("key_permissions")
...
def run(home, bot, keypath, call=request):
    ...
    try:
        ...
        fd = r.private_open(ledger, os.O_CREAT | os.O_RDWR)
        os.close(fd)
        ...
        for db in paths:
            if not db.exists():
                continue
            if db != home / "facts.db" and not (
                db.parent.parent.name == "shared"
                or re.fullmatch(r"private-[0-9a-f]{32}", db.parent.parent.name)
            ):
                continue
            r.fingerprint(db)
            st = db.stat()
            ...
            try:
                ...
                prior = c.execute(
                    "select inode,last_id from cursors where path=?", (str(db),)
                ).fetchone()
                if not prior or prior[0] != identity or prior[1] > mx:
                    c.execute(
                        "insert or replace into cursors values(?,?,?)", (str(db), identity, mx)
                    )
                    c.commit()
                    initialized += 1
                    continue
                rows = source.execute(
                    "select id,kind,fact from peer_facts where id>? and valid_to is null order by id limit 200",
                    (prior[1],),
                ).fetchall()
                for fid, kind, text in rows:
                    sentence = sanitize(text)
                    if sentence is None:
                        c.execute("update cursors set last_id=? where path=?", (fid, str(db)))
                        c.commit()
                        skipped += 1
                        continue
                    day = time.strftime("%Y-%m-%d", time.gmtime())
                    today = c.execute(
                        "select count(*) from reviews where day=?", (day,)
                    ).fetchone()[0]
                    if used >= 5 or today >= 100:
                        return {
                            "attempts": used,
                    ...
                    fp = hashlib.sha256(
                        (text + "\0" + MODEL + "\0" + json.dumps(question, sort_keys=True)).encode()
                    ).hexdigest()
                    c.execute(
                        "insert or ignore into reviews values(?,?,?,?,?,?,?,?,?)",
                        (str(db), fid, fp, day, "reserved", kind, None, None, MODEL),
                    )
                    ...
                    try:
                        ...
                        c.execute(
                            "update reviews set status=?,choice=?,confidence=? where path=? and id=? and fingerprint=?",
                            ("reviewed", a["choice"], a["confidence"], str(db), fid, fp),
                        )
                    except Exception:
                        c.execute(
                            "update reviews set status=? where path=? and id=? and fingerprint=?",
                            ("failed", str(db), fid, fp),
                        )
                        c.commit()
