...
r = module("feed-receipt")
j = module("journal-feed")
v = module("jev-review")
...
class Pipeline(unittest.TestCase):
    def setUp(self):
        ...
        self.home = self.p / "nunchi"
        self.home.mkdir(mode=0o700)
        self.bot = self.p / "bot"
        self.bot.mkdir(mode=0o700)
    ...
    def receipt(self, *args):
        with contextlib.redirect_stdout(io.StringIO()):
            return r.main(list(map(str, args)))

    def test_growing_session_and_failed_attempt(self):
        source = self.file("session with spaces.jsonl", "first")
        ledger = self.home / "receipts"
    ...
    def test_symlink_legacy_receipt_rejected(self):
        source = self.file("session", "a")
        other = self.file("other", "a")
        seen = self.p / "seen"
        seen.symlink_to(other)
        with self.assertRaises(OSError):
            self.receipt("due", self.home / "r", source, seen)

    def test_route_never_guesses_private_or_cross_scope(self):
        root = self.bot / "memory-audiences"
        ...
        for job in [
            {},
            {"memory_audience": "shared", "memory_scope": scope},
            {"memory_audience": "private", "memory_scope": "../escape"},
        ]:
            with self.assertRaises(ValueError):
                j.route(job, root)
        (root / scope).rmdir()
        (root / scope).symlink_to(self.home)
        with self.assertRaises(ValueError):
            j.route({"memory_audience": "private", "memory_scope": scope}, root)
    ...
    def test_journal_storage_failure_is_not_acknowledged(self):
        ...
        p.chmod(0o600)
        result = j.run(self.home, self.bot)
        self.assertEqual(result["failed"], 1)
        self.assertEqual(
            json.loads((self.home / "journal-receipts.jsonl").read_text())["status"], "failed"
        )
        ...
        self.assertEqual(c.execute("select count(*) from peer_facts").fetchone()[0], 1)
        c.close()
    ...
    def test_corrupt_global_db_does_not_block_private_review(self):
        ...
        scope = self.bot / "memory-audiences" / ("private-" + "a" * 32) / "nunchi"
        scope.mkdir(parents=True, mode=0o700)
        c = sqlite3.connect(scope / "facts.db")
        c.execute(
            "create table peer_facts(id integer primary key,kind text,fact text,valid_to text)"
        )
        c.close()
    ...
    def source(self):
        ...
        c.commit()
        c.close()
        (self.home / "facts.db").chmod(0o600)
    ...
    def add(self, db, n=1):
        with sqlite3.connect(db) as c:
            for i in range(n):
                c.execute(
                    "insert into peer_facts(kind,fact) values(?,?)",
                    ("decision", "The CI regression tests completed successfully after the patch."),
                )
    ...
    def test_jev_baseline_budget_dedup_and_source_unchanged(self):
        ...
        def call(sentence, key, q):
            calls.append(sentence)
            return {"choice": "task-progress", "confidence": 0.9}

        self.assertEqual(v.run(self.home, self.bot, key, call)["initialized"], 1)
        self.assertEqual(calls, [])
        self.add(db, 7)
        before = db.read_bytes()
        self.assertEqual(v.run(self.home, self.bot, key, call)["attempts"], 5)
        self.assertEqual(v.run(self.home, self.bot, key, call)["attempts"], 2)
        self.assertEqual(v.run(self.home, self.bot, key, call)["attempts"], 0)
        self.assertEqual(db.read_bytes(), before)
        with sqlite3.connect(self.home / "jev-review.db") as c:
            self.assertEqual(
                c.execute(
                    'select count(*) from reviews where choice="task-progress" and kind="decision"'
                ).fetchone()[0],
                7,
            )
    ...
    def test_jev_failed_call_not_replayed_and_no_memory_write(self):
        ...
        self.assertEqual(v.run(self.home, self.bot, key, fail)["attempts"], 0)
        self.assertEqual(db.read_bytes(), before)
    ...
    def test_privacy_and_response_schema(self):
        ...
        self.assertIsNone(
            v.sanitize(
                "SSH authorization uses ghp_1234567890abcdefghijklmnop for service account alice."
            )
        )
        self.assertIsNone(
            v.sanitize("Codex formatted Alice Smith’s divorce settlement for review.")
        )
        s = v.sanitize("nosuk CI deploy to https://example.com/private and 100.2.3.4 completed.")
        self.assertNotIn("example.com", s)
        self.assertNotIn("nosuk", s)
        self.assertNotIn("100.2", s)
        probs = {k: 1 / 10 for k in v.KINDS}
        raw = {
            "model": v.MODEL,
            "answers": {
                "kind": {
        ...
        v.validate(raw)
        raw["answers"]["kind"]["confidence"] = float("nan")
        with self.assertRaises(ValueError):
            v.validate(raw)
