...
- `skillsuggest` — 트랜스크립트 스캔→반복 절차 후보 제안(휴먼 승인 후 저작). 후보: `~/.claude/state/skill-candidates.md`.
...
- **작업 종결 시 '유휴 + 다음 후보' 상태로 클리어** — 스테일 목표가 컴팩션 후 재주입되면 다음 세션을 오도함 (14일 경과 시 재주입에 STALE 경고 배너, `CCC_CKPT_STALE_DAYS`로 조정/0=비활성)
