...
# Fleet-service lifecycle relaxation: pure lifecycle verbs on fleet units
# (a2a/hermes/openclaw/broker/gateway/worker/ccc-telegram-bridge) are allowed —
# locally or toward a peer node (ssh <node> …, systemctl -H <node> …).
...
run allow Bash command 'systemctl restart a2a-worker >/dev/null 2>&1'
run allow Bash command 'restart-worker'
run allow Bash command 'stop-broker'
# 타노드 재시작: the transport does not change the risk class — peer fleet units pass.
run allow Bash command 'ssh nosuk systemctl restart a2a-hermes-worker'
run allow Bash command 'ssh -p 2222 root@nosuk "systemctl restart hermes-broker"'
run allow Bash command 'systemctl -H nosuk restart a2a-broker'
run allow Bash command 'systemctl --host nosuk restart a2a-broker'
# ...but non-fleet units, config verbs, and ambiguity stay fail-closed.
run deny Bash command 'systemctl restart nginx'
run deny Bash command 'service nginx restart'
run deny Bash command 'ssh nosuk systemctl restart nginx'
run deny Bash command 'systemctl restart a2a-broker nginx'
run deny Bash command 'systemctl restart a2a-broker && systemctl restart nginx'
run deny Bash command 'systemctl restart'
run deny Bash command 'systemctl enable a2a-broker'
run deny Bash command 'systemctl disable --now a2a-broker'
run deny Bash command 'systemctl mask hermes-broker'
run deny Bash command 'systemctl disable --now ufw'
run deny Bash command 'systemctl daemon-reexec'
run deny Bash command 'systemctl isolate rescue.target'
run allow Bash command 'systemctl kill -s SIGTERM a2a-worker'
run deny Bash command 'systemctl kill -s SIGTERM sshd'
run deny Bash command 'pm2 delete gateway'
run deny Bash command 'pm2 restart my-api'
# Host lifecycle stays gated — local or remote (peer reboots need fresh approval).
...
# Container/orchestrator lifecycle stays gated (separate privilege boundary).
...
# Interpreter-mediated lifecycle cannot be parsed unambiguously → fail closed.
