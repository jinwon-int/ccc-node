...
5. **Respect the guard**: Fresh-Approval actions (deploy/secret/DB/force-push/release, non-fleet or local host lifecycle) are denied by `guard.py` (behind the `guard.sh` shim) and require operator approval (`CCC_ALLOW_GATED=1`). Fleet-service restarts (a2a/hermes/broker/gateway/worker/ccc-telegram-bridge, local or peer) and operations to allowlisted managed nodes are autonomous — see `hooks/RISK-PROFILES.md`.
