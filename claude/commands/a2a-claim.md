...
5. **Respect Fresh Approval**: Fresh-Approval actions (deploy/secret/DB/force-push/release, non-fleet or local host lifecycle) require operator approval — this is behavioral policy from `CLAUDE.md`, not a hook. Fleet-service restarts (a2a/hermes/broker/gateway/worker/ccc-telegram-bridge, local or peer) and operations to operator-managed nodes are autonomous — see the Fresh Approval / Service-control policy in `CLAUDE.md`.
