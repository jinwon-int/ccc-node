...
`agent-cron.sh run <task-id> --json` is an explicit manual execution path for due enabled tasks. It acquires/releases the local task lock, invokes `headless.sh`, and records `lastRunAt`/`lastStatus`/`lastRunId`. It still does not write push spool files, install schedulers, edit crontab/systemd, or send Telegram/provider messages.
