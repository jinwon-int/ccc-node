...
- recent audit (last 5): !`tail -5 ${CCC_STATE_DIR:-$HOME/.claude/state}/audit.jsonl 2>/dev/null || echo "(no audit log yet)"`
- approval-needed markers (last 5): !`tail -5 ${CCC_STATE_DIR:-$HOME/.claude/state}/approval-needed.log 2>/dev/null || echo "(none)"`
