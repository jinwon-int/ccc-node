...
- Responses with file paths matching a deliverable extension (documents, spreadsheets, presentations, data, archives, audio/video/images — not source code) are auto-sent as Telegram photos/documents. The path must resolve to a real file under `PROJECT_ROOT` and be < 50 MB (`_FILE_PATH_RE` / `_SENDABLE_FILE_EXTENSIONS` in `core/bot.py`, `MAX_SEND_FILE_BYTES` in `core/bot_delivery.py`). Paths outside `PROJECT_ROOT` are not auto-sent.
