...
| `CCC_BRIDGE_CLAUDE_UNRESTRICTED` | No | Codex parity for the Claude path (default **true**, `owner-operator` only — ignored on every other profile). At the default, a non-root `owner-operator` node runs Claude with `permission_mode=bypassPermissions`, no OS sandbox, and no host settings chain, so not even the native `permissions.deny` backstop loads — matching Codex's `never + dangerFullAccess`. Memory context is preserved. Set **false** to keep the native `permissions.deny` backstop as the boundary; reversible per node. **Ignored under root** (Claude Code refuses `bypassPermissions` with root/sudo) — the flag degrades to the guarded path with a logged warning; run the bridge non-root to gain the unrestricted posture there |
