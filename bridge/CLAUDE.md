...
| `CCC_USAGE_KIMI_5H_REQUEST_LIMIT` | No | Operator-configured Kimi Code 5-hour request limit (from the Kimi Code Console); when set, `/usage` shows `used/limit req · N left` for the synthesized `Kimi 5-hour` window (local estimate). Unset = count-only |
