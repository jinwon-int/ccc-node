...
| `CCC_DEAD_SESSION_WAKEUP` | No | Opt-in dead-session wakeup (#364 P2, default **false**): when a background task completes after its Claude session died, resume the session for one bounded autonomous turn so the CLI processes its pending task notifications. Metered as autonomous spend, gated by the #388 daily budget, per-conversation cooldown + per-session attempts cap. Claude provider only; requires `CCC_RESUME_PERSISTED_SESSIONS` |
