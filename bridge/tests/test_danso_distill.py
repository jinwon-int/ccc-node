...
def journal(root):
    ...
    rows = [
        {"type": "session", "version": 3, "id": ident},
        {
            "type": "message",
            "id": "user-1",
...
def test_snapshot_pins_scope_identity_permissions_and_lock(tmp_path):
    ...
    result = read_danso_snapshot(root, ident, bounds=TranscriptBounds())
    assert result.thread_hash == hashlib.sha256(ident.encode()).hexdigest()
    assert result.messages[0].role == "user"
    with path.open() as file:
        fcntl.flock(file, fcntl.LOCK_EX | fcntl.LOCK_NB)
        with pytest.raises(BlockingIOError):
            read_danso_snapshot(root, ident, bounds=TranscriptBounds())
    path.chmod(0o644)
    ...
    with pytest.raises(OSError):
        read_danso_snapshot(link, ident, bounds=TranscriptBounds())
    with pytest.raises(SnapshotUnavailableError):
        read_danso_snapshot(tmp_path / "shared", ident, bounds=TranscriptBounds())
    with pytest.raises(ValueError):
        read_danso_snapshot(root, "../escape", bounds=TranscriptBounds())
...
def test_snapshot_rejects_partial_unresolved_or_redirected(tmp_path, damage):
    ...
    if damage == "partial":
        ...
        path.write_text(path.read_text().replace(ident, str(uuid.uuid4())))
    else:
...
@pytest.mark.anyio
async def test_extractor_uses_only_explicit_auth_and_tool_free_private_input(tmp_path, monkeypatch):
    ident, _ = journal(tmp_path / "private")
    snapshot = read_danso_snapshot(tmp_path / "private", ident, bounds=TranscriptBounds())
...
def test_danso_facts_persist_idempotently_in_the_bound_audience(tmp_path):
    ...
    data = build_extraction_input(
        read_danso_snapshot(tmp_path / "journal", ident, bounds=TranscriptBounds()),
        provider="danso",
