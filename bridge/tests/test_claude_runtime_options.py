...
def test_audience_scoped_memory_fails_closed(tmp_path: Path) -> None:
    # SessionRequest carries no Telegram route, so audience-scoped memory
    # cannot resolve an audience here. Fail closed rather than silently
    # starting the session without the memory the node was configured for:
    # an audience-scoped node must not degrade to no-memory unnoticed.
    runtime = ClaudeRuntime(
