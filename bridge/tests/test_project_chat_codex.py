...
@pytest.mark.anyio
async def test_project_chat_binds_opaque_claude_audiences_at_request_time(
    tmp_path: Path,
) -> None:
    ...
    assert private_env["CCC_MEMORY_AUDIENCE"] == "private"
    assert shared_env["CCC_MEMORY_AUDIENCE"] == "shared"
    assert private_env["CCC_STATE_DIR"] != shared_env["CCC_STATE_DIR"]
    assert private_env["CCC_MEMORY_SHARED_STATE_DIR"] == shared_env["CCC_STATE_DIR"]
    assert "CODEX_HOME" not in private_env
    assert "CODEX_HOME" not in shared_env
    serialized = repr((private_env, shared_env))
    assert "934719283" not in serialized
    assert "-100456" not in serialized
...
async def test_codex_keeps_typing_alive_and_shows_tool_heartbeat(
    ...
    # deterministic under load instead of racing the heartbeat timer.
