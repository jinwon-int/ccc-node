...
- Codex `auto-approve` (the package default) sends `approvalPolicy=never`, no reviewer, and `sandboxPolicy={type: workspaceWrite, networkAccess: false}` on every turn. Approval prompts are suppressed, but the workspace sandbox remains explicit; boundary operations may fail instead of prompting. This mode never requests `dangerFullAccess`.
