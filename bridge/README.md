...
- `CCC_BRIDGE_BASH_POLICY` controls approval UX inside the selected boundary: `auto-approve` (default), `approve-each`, or `disabled`. Approval never widens `strict-project`, and it is not a second sandbox in `owner-operator`. Claude maps `approve-each` to one Telegram confirmation per Bash command. Codex maps it to app-server `approvalPolicy=untrusted`: trusted commands such as `ls`, `cat`, and `sed` run without a prompt, while untrusted command execution, file changes, and permission escalation use the owner-only Telegram approval UI.
- Codex provider approvals are delivered only to the single allowlisted owner. They are turn-scoped and offer Allow or Deny only; Codex has no session-wide **Allow All** path. `auto-approve` maps to Codex `never`; disabled policy requests are denied without rendering approval buttons.
