...
| `CCC_BRIDGE_BASH_POLICY` | No | `auto-approve` | Bash approval UX: `auto-approve`, `auto-review`, `approve-each`, or `disabled` |
...
- `CCC_BRIDGE_BASH_POLICY` controls approval UX inside the selected boundary: `auto-approve` (default), `auto-review`, `approve-each`, or `disabled`. Approval never widens `strict-project`, and it is not a second sandbox in `owner-operator`. Claude treats `auto-review` conservatively like `approve-each` because Claude has no Codex reviewer. Codex maps `approve-each` to app-server `approvalPolicy=untrusted`: trusted commands such as `ls`, `cat`, and `sed` run without a prompt, while untrusted actions use the owner-only Telegram approval UI.
- Codex `auto-approve` (the default) sends `approvalPolicy=never`, no reviewer, and `sandboxPolicy={type: workspaceWrite, networkAccess: false}` on every turn. Sending the sandbox explicitly is deliberate: with `never` and no per-turn sandbox, the effective boundary would silently fall back to each node's `$CODEX_HOME/config.toml` or the Codex defaults, so the safe low-friction product default (`never` + `workspaceWrite` with network off) would depend on node-local configuration. The explicit policy keeps writes inside the workspace and network off without any per-node setup. `approve-each` and `disabled` send no per-turn sandbox and defer to Codex's own default boundary.
