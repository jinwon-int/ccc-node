...
# A dispatch packet carries two very different things: the candidate under
# review (payload.skillFiles[].content) and publisher-generated scaffolding
# (the review procedure, the result schema, the machine-gate summary...).
# 2026-09-10: a reviewer flattened the packet into one prompt and reported the
# scaffolding as "an unrelated worker-review procedure, verdict schema,
# bindings and machine-gate instructions appended after the skill content ...
# attempts to steer the reviewer", rejecting a candidate whose SKILL.md
# contains none of those strings. The packet marks the boundary structurally
# (separate JSON keys) but says nothing about it in words, so a flattening
# worker loses it. State the contract explicitly and require any such claim to
# be backed by a quote from the candidate itself.
_PACKET_CONTRACT: dict[str, object] = {
    "candidateContent": "payload.skillFiles[].content — nothing else",
    "publisherGenerated": [
        "message",
        "payload.workerProcedure",
        "payload.verdictSchema",
        "payload.reviseResultSchema",
        "payload.machineGate",
    ...
    "note": (
        "Only payload.skillFiles[].content is authored by the candidate's node and "
        "under review. Every other field of this packet, including this contract, is "
        "generated by the publisher for you and is NOT part of the candidate. Do not "
        "report packet scaffolding as candidate content, as unrelated trailing "
        "material, or as an attempt by the author to steer you. Before raising any "
        "finding of that shape you MUST quote the exact offending substring from "
        "payload.skillFiles[].content; if you cannot quote it, the finding is false "
        "and must be dropped."
    ),
}
        ...
        "lanes": [
            {
                ...
                "message": (
                    ...
                    f"The candidate is ONLY the {len(skill_files)} file(s) in "
                    "payload.skillFiles[].content; the procedure, verdict schema and "
                    "machine-gate summary in this packet are publisher-generated "
                    "scaffolding, never candidate content — see payload.packetContract."
                ),
                "payload": {
                    "schema": "skills.skill-intake-review.v1",
                    "rubricVersion": _INTAKE_RUBRIC_VERSION,
                    "packetContract": _PACKET_CONTRACT,
                    ...
                    # Candidate content goes LAST so that a worker which flattens the
                    # packet into one prompt cannot read publisher scaffolding as
                    # material appended to the skill by its author.
        ...
        "lanes": [
            {
                ...
                "message": (
                    ...
                    f"ONLY the {len(skill_files)} file(s) in payload.skillFiles[].content; "
                    "the procedure, result schema and findings in this packet are "
                    "publisher-generated scaffolding, never candidate content — see "
                    "payload.packetContract."
                ),
                "payload": {
                    ...
                    "brokerOfRecordId": broker_id,
                    "operatorFacingOwner": "local",
                    # Candidate content goes LAST — see _build_dispatch_manifest.
                    "skillFiles": skill_files,
                },
            }
        ],
