...
ok "setup installs the canonical pending distill journal for the hook CLI" \
  '[ -f "$rewrite_claude/hooks/ccc_distill_pending_journal.py" ] && [ ! -x "$rewrite_claude/hooks/ccc_distill_pending_journal.py" ] && cmp -s "$ROOT/bridge/memory/distill_pending_journal.py" "$rewrite_claude/hooks/ccc_distill_pending_journal.py" && [ -x "$rewrite_claude/hooks/distill/pending-job.py" ]'
...
ok "installed pending distill CLI imports its canonical colocated journal" \
  'PYTHONDONTWRITEBYTECODE=1 PYTHONPATH= python3 "$rewrite_claude/hooks/distill/pending-job.py" --help >/dev/null 2>&1'
