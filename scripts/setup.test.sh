...
HOME="$TMP/root-bypass-home" CCC_CLAUDE_DIR="$root_claude" CCC_HERMES_DIR="$TMP/root-bypass-hermes" \
  CCC_SETUP_GUARD_PROFILE_PATH="$TMP/root-guard-profile" \
...
ok "fresh root install seeds the operational-relax profile" \
...
# Re-runs and explicit strict installs must not broaden policy unexpectedly.
printf '%s\n' '# operator strict choice' > "$TMP/root-guard-profile"
chmod 0600 "$TMP/root-guard-profile"
root_profile_before="$(sha256sum "$TMP/root-guard-profile")"
HOME="$TMP/root-bypass-home" CCC_CLAUDE_DIR="$root_claude" CCC_HERMES_DIR="$TMP/root-bypass-hermes" \
  CCC_SETUP_GUARD_PROFILE_PATH="$TMP/root-guard-profile" \
  PATH="$TMP/root-bin:$PATH" bash "$SETUP" --no-backup >/dev/null 2>&1
ok "root re-run preserves an existing operator guard profile byte-for-byte" \
  '[ "$(sha256sum "$TMP/root-guard-profile")" = "$root_profile_before" ] && [ "$(stat -c %a "$TMP/root-guard-profile")" = 600 ]'
...
HOME="$TMP/strict-root-home" CCC_CLAUDE_DIR="$strict_claude" CCC_HERMES_DIR="$TMP/strict-root-hermes" \
  ...
  PATH="$TMP/root-bin:$PATH" bash "$SETUP" --no-backup --strict-guard >/dev/null 2>&1
...
mkdir -p "$TMP/nonroot-bin"
cat > "$TMP/nonroot-bin/id" <<'STUB'
#!/bin/sh
[ "$1" = "-u" ] && { echo 1001; exit 0; }
exec /usr/bin/id "$@"
STUB
chmod +x "$TMP/nonroot-bin/id"
HOME="$TMP/fresh-nonroot-home" CCC_CLAUDE_DIR="$TMP/fresh-nonroot-claude" CCC_HERMES_DIR="$TMP/fresh-nonroot-hermes" \
  ...
  PATH="$TMP/nonroot-bin:$PATH" bash "$SETUP" --no-backup >/dev/null 2>&1
ok "fresh non-root install never writes an operational-relax profile" \
