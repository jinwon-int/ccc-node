...
cp "$ROOT"/claude/hooks/nunchi/{codex-loader.py,nunchi.py,judge-batch.py,wiki-promote.py,judge-verdict.schema.json,codex-feed.sh,piri-feed.sh,feed-receipt.py,session-tail.py,danso-feed.sh,ingest-cron.sh,bridge-journal.py,feed-common.sh,bench.sh,bench-qset.tsv,sessionstart.sh,mempalace-refresh.sh} "$hooks/nunchi/"
