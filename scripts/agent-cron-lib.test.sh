...
if python3 "$HERE/agent_cron_lib_test.py" >"$out_file" 2>&1 && \
   python3 "$HERE/agent_cron_schema_test.py" >>"$out_file" 2>&1 && \
   python3 "$HERE/agent_cron_persist_test.py" >>"$out_file" 2>&1 && \
   python3 "$HERE/agent_cron_exec_plan_test.py" >>"$out_file" 2>&1; then
