...
# The recovery snapshot deliberately outlives setup: a service that fails to
# come back is exactly when rollback material is needed, and deleting it here
# left that path with nothing to restore from. It is removed once the restarts
# have succeeded (below), so the success path keeps its old no-residue
# behavior.
