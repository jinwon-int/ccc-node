...
writeback_queue_json() {
  ...
  for path in "${paths[@]}"; do
    ...
    safe="$(jq -ce --arg expected_id "${name%.json}" '
      ...
      | select(
          ...
          and ((.honcho_sink_status // null) | . == null or oneof(["pending","running","retryable_failed","done","terminal_failed","disabled"]))
          ...
          and ((.honcho_sink_attempts // 0) | nnint)
      ...
      | (.honcho_sink_status // null) as $honcho
      ...
      | {
          ...
          honcho_retries:(.honcho_sink_attempts // 0),
          accounting:(
          ...
          pending:(
            ...
            or ($status == "extraction_done" and ($honcho | oneof(["pending","running","retryable_failed"])))
          ),
          degraded:(
  ...
  printf '%s\n' "${records[@]}" | jq -cs \
    ...
    --argjson record_bytes "$record_bytes" '
      ...
      | ([ $jobs[] | .honcho_retries ] | add // 0) as $honcho_retries
      ...
      | {
          ...
          retries:{
            ...
            local:$local_retries,
            wiki:$wiki_retries,
            honcho:$honcho_retries,
            total:($snapshot_retries + $extraction_retries + $local_retries + $wiki_retries + $honcho_retries)
          },
          accounting:{
          ...
          honcho_status_counts:(reduce ($jobs[] | select(.honcho_status != null)) as $job ({}; .[$job.honcho_status] = ((.[$job.honcho_status] // 0) + 1)))
        }
    '
}
